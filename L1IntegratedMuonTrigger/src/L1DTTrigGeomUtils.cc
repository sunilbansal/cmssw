/*
 * \file L1DTTrigGeomUtils.cc
 * 
 * This is a modified copy of the DTTrigGeomUtils.cc file from the CMSSW
...
L1DTTrigGeomUtils::L1DTTrigGeomUtils(ESHandle<DTGeometry> muonGeom, bool dirInDeg)
: muonGeom_(muonGeom)
{
...
L1DTTrigGeomUtils::~L1DTTrigGeomUtils() {}


void L1DTTrigGeomUtils::computeSCCoordinates(const DTRecSegment4D* track, int& scsec, float& x, float& xdir, float& y, float& ydir)
{
...
void L1DTTrigGeomUtils::phiRange(const DTChamberId& id, float& min, float& max, int& nbins, float step)
{
...
void L1DTTrigGeomUtils::thetaRange(const DTChamberId& id, float& min, float& max, int& nbins, float step)
{
...
float L1DTTrigGeomUtils::trigPos(const L1MuDTChambPhDigi* trig)
{
	int wh   = trig->whNum();
...
float L1DTTrigGeomUtils::trigDir(const L1MuDTChambPhDigi* trig)
{
	int wh   = trig->whNum();
	int sec  = trig->scNum()+1;
	int phi  = trig->phi();
